...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    def timesAcc(chars: List[Char],
      ...
      else timesAcc(chars.tail, acc ::: List((chars.head, 1)))
    }
  ...
  def createCodeTree(chars: List[Char]): CodeTree =
    until(trees => singleton(trees), trees => combine(trees))(
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table.find(pair => pair._1 == char).head._2
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def add(bit: Bit, codeTable: CodeTable): CodeTable = {
      codeTable.map(pair => (pair._1, bit :: pair._2))
    }
    add(0, a) ::: add(1, b) // adds 0 and 1 to to all codes to merge trees
  }
